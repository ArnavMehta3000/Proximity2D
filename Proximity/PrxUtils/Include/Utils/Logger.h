...
namespace Proximity::Utils
{
	...
	public:
		...
		static void SetFileOutput(const char* filePath);
		...
		{
			Log(LOG_LEVEL_DEBUG, msg, args...);  // Don't log line and filename
		}

		template<typename... Args>
		static void Info(int line, const char* filename, const char* msg, Args... args)
		{
			Log(LOG_LEVEL_INFO, msg, args...);  // Don't log line and filename
		}
